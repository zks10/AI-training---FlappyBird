...
BIRD_IMGS = [
    pygame.transform.scale2x(pygame.image.load(os.path.join(img_path, "bird1.png"))),
...
class Bird:
    ...
    def move(self):
        ...
        if d < 0 or self.y < self.height + 35:
...
class Pipe:
    ...
    def set_height(self):
        self.height = random.randrange(randTop, randBot)
...
def main():
    while True:  # Infinite loop for restarting
        bird = Bird(230, 350)
        base = Base(730)
        pipes = [Pipe(600)]
        ...
        while run:
            ...
            for pipe in pipes:
                if pipe.collide(bird):  # End game on collision
                    run = False
            ...
            if bird.y + bird.img.get_height() >= 730 or bird.y < 0:  # End game if bird hits ground or flies out of bounds
                run = False

            base.move()
            draw_window(win, bird, pipes, base, score)

        # Restart the game after a small delay
        time.sleep(1)
main()